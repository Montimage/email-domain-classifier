...
"""
    ...
    def print_domain_stats(self, domain_counts: Dict[str, int], total: int):
        ...
        for domain, count in sorted_domains:
            ...
            # Get domain style
            style = f"domain.{domain}" if domain in [
                'finance', 'technology', 'retail', 'logistics', 
                'healthcare', 'government', 'hr', 'telecommunications',
                'social_media', 'education', 'unsure'
            ] else "white"
    ...
    def print_summary_panel(self, stats: Dict):
        ...
        content.append(f" ({summary.get('classification_rate_percent', 0)}%)\n", style="green")
        ...
        content.append(f"{timing.get('emails_per_second', 0):.0f} emails/sec\n", style="white")
        
        content.append("\n📈 Quality Metrics\n\n", style="bold cyan")
        
        content.append("  Agreement Rate:    ", style="dim")
        agreement = quality.get('method_agreement_rate', 0)
        agreement_style = "green" if agreement > 70 else "yellow" if agreement > 50 else "red"
