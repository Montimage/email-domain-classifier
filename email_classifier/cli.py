...
Output:
  ...
  Also generates classification_report.txt and classification_report.json.
    ...
    parser.add_argument(
        'input',
        type=str,
        help='Path to input CSV file containing email dataset'
    )
    ...
    parser.add_argument(
        '--no-report',
        action='store_true',
        help='Skip generating summary reports'
    )
    ...
    # Setup logging
    log_file = Path(args.log_file) if args.log_file else output_dir / 'classification.log'
            ...
            def progress_callback(current: int, total: int, status: str):
                ...
                if progress is not None:
                    ...
                    if task_id is not None:
                        progress.update(task_id, completed=current, description=f"[cyan]{status}")
