    ...
    def _compile_patterns(self):
        ...
        for name, profile in self.domains.items():
            self._sender_patterns[name] = [
                ...
                for p in profile.sender_patterns
            ]
            self._subject_patterns[name] = [
                re.compile(p, re.IGNORECASE) 
                for p in profile.subject_patterns
            ]
...
class StructuralTemplateClassifier:
    ...
    # Greeting patterns
    GREETING_PATTERNS = [
        re.compile(r'^(dear|hello|hi|greetings|good\s+(morning|afternoon|evening))', re.IGNORECASE | re.MULTILINE),
    ]
    
    # Signature patterns
    SIGNATURE_PATTERNS = [
        re.compile(r'(sincerely|regards|best|thank you|thanks|cheers),?\s*\n', re.IGNORECASE),
        re.compile(r'\n[-–—]{2,}\s*\n', re.MULTILINE),
    ...
    # Disclaimer patterns
    DISCLAIMER_PATTERNS = [
        re.compile(r'(confidential|disclaimer|privileged|intended recipient)', re.IGNORECASE),
    ...
    FORMAL_INDICATORS = [
        'pursuant', 'hereby', 'aforementioned', 'regarding', 'enclosed',
        'please find', 'kindly', 'respectfully', 'we regret', 'be advised'
    ]
    
    CASUAL_INDICATORS = [
        'hey', 'thanks!', 'awesome', 'cool', 'btw', 'fyi', 'asap',
        'gonna', 'wanna', 'gotta', ':)', '!!'
    ]
    ...
    def _analyze_sender_structure(self, sender: str) -> Dict:
        ...
        dept_patterns = ['support', 'billing', 'sales', 'info', 'contact', 
                        'admin', 'help', 'service', 'team', 'notifications']
    ...
    def _score_template_match(
        ...
        elif (features['formality'] == 'semi-formal' or 
              profile.formality_level == 'semi-formal'):
        ...
        elif sender_features['is_noreply'] and profile.name in ['technology', 'retail', 'logistics']:
